...
async function readDB(){ try{ return JSON.parse(await fs.readFile(mailDB,"utf8")); }catch{ return []; } }
async function writeDB(d){ await fs.writeFile(mailDB, JSON.stringify(d, null, 2)); }
async function logSentMail(name, type){
  const data = await readDB(); const i = data.findIndex(e=>e.name===name); const now=new Date().toISOString();
  if(i!==-1){ data[i].sent=true; data[i].sentAt=now; data[i].lastType=type; }
...
app.post("/send-docx", requireApiKey, async (req,res)=>{
  try{
    ...
    await transporter.sendMail({
      ...
      attachments: [{ filename:`harmonogram_${cleanName}.docx`, content:buf,
